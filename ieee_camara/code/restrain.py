...
parser.add_argument('-cm', '--classifier', type=str, default='resnet152', help='Base classifier model to use')
...
if not (args.test or args.test_train):
    ...
    scheduler = ReduceLROnPlateau(optimizer, factor=0.3, patience=10, min_lr=1e-9, epsilon=1e-5, verbose=1, mode='min')
    looker = EarlyStopping(patience=30) 
    best_val_loss = None
    train_and_validate(
        train_loader,
        valid_loader,
    ...
    with conditional(args.test, open(csv_name, 'w')) as csvfile:
        ...
        for i, idx in enumerate(tqdm(ids)):
            ...
            img_batch         = np.zeros((len(transforms)* 2 * 2, CROP_SIZE, CROP_SIZE, 3), dtype=np.float32)
            manipulated_batch = np.zeros((len(transforms)* 2 * 2, 1),  dtype=np.float32)
