    ...
    for epoch in range(start_epoch, epochs+1):
        ...
        if True:
            save_checkpoint(
                ...
                '{experiment_name}_{epoch}_{loss}_{centerloss}_{acc}_{val_acc}.pth'.format(experiment_name=experiment_name, epoch=epoch, loss=val_loss, centerloss=center_loss, acc=acc,val_acc=val_acc),
                checkpoint_dir,
            )
            best_val_loss = val_loss
        scheduler.step(val_loss, epoch)
        if looker.step(val_loss, epoch):
            break
    return model
...
def train(train_loader, model, optimizer, criterion, epoch,alpha,center_loss_weight):
    ...
    for i, (inputs, O, targets) in enumerate(train_loader):
        ...
        center_loss, model.centers = model.get_center_loss(targets, alpha)
        cross_loss = criterion(outputs, targets)
        loss = center_loss_weight*center_loss + cross_loss 
        #loss = cross_loss 
        optimizer.zero_grad()
        batch_size = inputs.size(0)
        (batch_size * loss).backward()
        optimizer.step()
        center_losses.append(center_loss.data[0]) 
        losses.append(cross_loss.data[0])
        accuracy_scores += list(targets.data.cpu().numpy() == np.argmax(outputs.data.cpu().numpy(), axis=1))
        
        if i % 50 == 0:
            print('Step: {}, train_loss: {}, center_loss: {}'.format(i, np.mean(losses[-50:]), np.mean(center_losses[-50:])))
        #if i % 100 == 0:
        #    logging.info('Step: {}, train_loss: {}'.format(i, np.mean(losses[-100:])))
            
    train_loss = np.mean(losses)
    center_mean_loss = np.mean(center_losses)
    train_accuracy = np.mean(accuracy_scores)
    print('train_loss: {}, center_loss: {}, train_acc: {}'.format(train_loss, center_mean_loss,train_accuracy))
    logging.info('train_loss: {},center_loss: {},  train_acc: {}'.format(train_loss, center_mean_loss,train_accuracy))
    return train_loss, center_mean_loss, train_accuracy
