...
class IEEECameraDataset(Dataset):
    def __init__(self, items, crop_size, verbose=False, training=True, model=None):
    ...
    def __getitem__(self, idx):
        sample = process_item(self.items[idx], self.crop_size, self.verbose, training=self.training, transforms=self.transforms, model=self.model)
        if sample is None:
...
def process_item(item, crop_size, verbose, training, transforms=[[]], model=None):
    ...
    for transform in transforms:
        ...
        img = preprocess_image(img, model)# TODO:
        if verbose:
...
def preprocess_image(img, model):
    if model is None:
        print('model is None!!!!!')
    #print(model)
    if model in ['nasnetalarge', 'xception', 'inceptionresnetv2']:    
        mean = [0.5, 0.5, 0.5]
        std = [0.5, 0.5, 0.5]
    if model in ['resnet152','densenet201','resnext101_64x4d']:
