...
rgf = RGFClassifier(   # See https://www.kaggle.com/scirpus/regularized-greedy-forest#241285
                    max_leaf=1100,  # Parameters suggested by olivier in link above
                    algorithm="RGF",  
                    loss="Log",
                    l2=0.01,
                    sl2=0.01,
...
val.to_csv('%s/the1ow_xgb_valid.csv' % config.sub_folder, float_format='%.6f', index=False)

# Create submission file
sub = pd.DataFrame()
sub['id'] = id_test
sub['target'] = y_test_pred
sub.to_csv('%s/the1ow_xgb_submit.csv' % config.sub_folder, float_format='%.6f', index=False)
