...
for i, (f_ind, outf_ind) in enumerate(kfold.split(X_train, y_train)):
    ...
    cb = roc_auc_callback(training_data=(proc_X_train_f, y_train_f.values),
                          validation_data=(proc_X_val_f, y_val_f.values))
    for j in range(runs_per_fold):
        print("\nrun %s begin:" % j) 
        NN = build_embedding_network()
        NN.fit(proc_X_train_f, y_train_f.values, epochs=n_epochs, batch_size=4096, verbose=2, callbacks=[cb])
