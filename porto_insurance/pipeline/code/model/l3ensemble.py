...
feat_names.append("nb9")
feat_names.append("lda9")
#feat_names.append("qda")
...
for i, feat_name in enumerate(feat_names):
  train = pd.read_csv("../subm/ensemble/%s%s" % ( feat_name,pred_path))
  test = pd.read_csv("../subm/ensemble/%s%s" % ( feat_name,subm_path))
...
val.to_csv('../subm/ensemble/%s8_valid.csv' % log, float_format='%.6f', index=False)

# Create submission file
sub = pd.DataFrame()
sub['id'] = id_test
sub['target'] = y_test_pred
sub.to_csv('../subm/ensemble/%s8_submit.csv' % log, float_format='%.6f', index=False)
