...
def GetData(strdirectory):
    ...
    train = blindloodata.sort_values(by='id')
...
def main():
    ...
    y_valid_pred = GPAri(gptrain)
    y_test_pred = GPAri(gptest)
    print('GPAri Gini Score:', GiniScore(gptrain.target,y_valid_pred))

    # Calibrate noise scale (si on veut)
    NOISE_SCALE = .21
    i = 2
    np.random.seed(i)
    logOdds = y_valid_pred.apply(lambda x: np.log(x/(1-x)))
    noise = np.random.normal(0,NOISE_SCALE,y_valid_pred.shape[0])
    valid1 = (logOdds+noise).apply(lambda x: 1/(1+np.exp(-x)))
    gin0 = GiniScore(gptrain.target,valid1)
    print('Average Gini Score:', gin0)
    # Save validation predictions for stacking/ensembling
    val = pd.DataFrame()
    val['id'] = gptrain['id'].values
    val['target'] = valid1.values
    val.to_csv('%s/scirp_gp_valid.csv' % config.sub_folder, float_format='%.6f', index=False)

    # Create submission file
    sub = pd.DataFrame()
    sub['id'] = gptest['id'].values
    sub['target'] = y_test_pred.values
    sub.to_csv('%s/scirp_gp_submit.csv' % config.sub_folder, float_format='%.6f', index=False)
