...
def combine_feat(feat_names, feat_path_name, dfTrain, dfTest, skf):
  ...
  ## so we switch trainInd and validInd
    for fold, (validInd, trainInd) in enumerate(skf[run-1]):
      ...
      X_train = dfTrain[col].values[trainInd]
      X_valid = dfTrain[col].values[validInd]
      Y_train = dfTrain["target"].values[trainInd]
      Y_valid = dfTrain["target"].values[validInd]
  ...
  col = [c for c in dfTrain.columns if c not in ['id','target']]
  col = [c for c in col if not c.startswith('ps_calc_')]
  X_train = dfTrain[col].values
  X_test = dfTest[col].values
  Y_train = dfTrain["target"].values
  Y_test = np.ones((dfTest.shape[0]))
