...
blend = np.concatenate([X.values,test_df.values])
...
test_df = pd.DataFrame(blend[X.shape[0]:,:],columns=X.columns)

# Set up folds
K = 5
kf = KFold(n_splits = K, random_state = 1108, shuffle = True)
log_model = SVC(C=1,probability=True)
np.random.seed(1108)
global cv
cv = [[],1,1]
def get_cv(col_name):
...
result_train.to_csv('../mixxgb_train.csv', float_format='%.6f', index=False)

# Create submission file
result_test.to_csv('../mixxgb_test.csv', float_format='%.6f', index=False)
