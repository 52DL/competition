...
for m in [0]:
    for n in [1]:
        ...
        val.to_csv('../subm/picf+xgb_valid.csv', float_format='%.6f', index=False)

        # Create submission file
        sub = pd.DataFrame()
        sub['id'] = id_test
        sub['is_iceberg'] = y_test_pred
        sub.to_csv('../subm/picf+xgb_submit.csv', float_format='%.6f', index=False)
