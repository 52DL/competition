...
for rs in [7,8,9,10]:
    ...
    X,look,y,ylook=train_test_split(X,y,shuffle=True,test_size=.2,random_state=rs)
    y_train_pred = 0.0*y.values
    y_valid_pred = 0.0*y.values
    y_look_pred = 0.0*ylook.values
    y_test_pred = .00
    ...
    for m in [0]:
        for n in [0]:
            ...
            for i, (train_index, test_index) in enumerate(kf.split(X)):
              ...
              pred = fit_model.predict_proba(X_train, ntree_limit=xgbmodel.best_ntree_limit+0)[:,1]
              print( "  train Gini = ", log_loss(y_train, pred) )
              y_train_pred[train_index] += pred

              pred = fit_model.predict_proba(X_valid, ntree_limit=xgbmodel.best_ntree_limit+0)[:,1]
              print( "  valid Gini = ", log_loss(y_valid, pred) )
              y_valid_pred[test_index] = pred

              pred = fit_model.predict_proba(look, ntree_limit=xgbmodel.best_ntree_limit+0)[:,1]
            ...
            y_train_pred /= K-1  # Average test set predictions
            #y_valid_pred  # Average test set predictions
            y_look_pred /= K  # Average test set predictions
            y_test_pred /= K  # Average test set predictions

            print( "\nGini for full train set:" ,log_loss(y, y_train_pred),accuracy_score(y,np.round(y_train_pred)))
            print( "\nGini for full valid set:" ,log_loss(y, y_valid_pred),accuracy_score(y,np.round(y_valid_pred)))
            print( "\nGini for full look set:" ,log_loss(ylook, y_look_pred),accuracy_score(ylook,np.round(y_look_pred)))
            '''
