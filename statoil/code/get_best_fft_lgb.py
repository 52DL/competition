...
train = pd.merge(sbmr_train, fft_train, how='left',on='id')
...
for m in [5]:
    for n in [0]:
        ...
        val.to_csv('../subm/fft+lgb_valid.csv', float_format='%.6f', index=False)

        # Create submission file
        sub = pd.DataFrame()
        sub['id'] = id_test
        sub['is_iceberg'] = y_test_pred
        sub.to_csv('../subm/fft+lgb_submit.csv', float_format='%.6f', index=False)
