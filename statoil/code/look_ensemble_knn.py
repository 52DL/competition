...
for rs in [7,8,9,10]:
    ...
    for i, (train_index, test_index) in enumerate(kf.split(X)):
      ...
      pred = fit_model.predict_proba(X_train)[:,1].clip(0.001,.999)
      print( "  train Gini = ", log_loss(y_train, pred) )
      y_train_pred[train_index] += pred
      pred = fit_model.predict_proba(X_valid)[:,1].clip(0.001,.999)
      ...
      probs = fit_model.predict_proba(X_test)[:,1].clip(.001,.999)
      y_test_pred += probs
