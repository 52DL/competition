...
ir_train = pd.read_csv('../feat/ircpu_train.csv')
ir_test = pd.read_csv('../feat/ircpu_test.csv')
...
for m in [0]:
    for n in [1]:
        ...
        for i, (train_index, test_index) in enumerate(kf.split(X,y)):
            ...
            for j in range(RUNs):
                ...
                if OPTIMIZE_XGB_ROUNDS:
                    ...
                    params['seed'] = seed
                    fit_model = xgb.train(params, xgb.DMatrix(X_train, y_train), 2000,  watchlist, verbose_eval=500, early_stopping_rounds=300)
        ...
        val.to_csv('../subm/ir+xgb_valid.csv', float_format='%.6f', index=False)

        # Create submission file
        sub = pd.DataFrame()
        sub['id'] = id_test
        sub['is_iceberg'] = y_test_pred
        sub.to_csv('../subm/ir+xgb_submit.csv', float_format='%.6f', index=False)
